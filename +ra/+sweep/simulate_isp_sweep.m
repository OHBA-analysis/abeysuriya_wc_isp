classdef simulate_isp_sweep < Pepper
	...
	methods 
		...
		function self = simulate_isp_sweep(network,unit,velocity,coupling,isp_target)
			self.unit = unit;
		...
		function [inputs,task_ids] = get_inputs(self)
			[inputs.c,inputs.v,inputs.isp_target] = meshgrid(self.coupling,self.velocity,self.isp_target);
		...
		function out = do_work(self,task_id,inputs)
			...
			u = self.unit;
			u.I.isp_target = inputs.isp_target(task_id);
