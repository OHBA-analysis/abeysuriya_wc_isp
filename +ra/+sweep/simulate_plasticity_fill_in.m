classdef simulate_plasticity_fill_in < Pepper
	...
	methods 
		...
		function self = simulate_plasticity_fill_in(network,unit,velocity,coupling,isp_target,original_folder)
			self.unit = unit;
		...
		function [inputs,task_ids] = get_inputs(self)
			...
			d = load(fullfile('/vols/Scratch/romesh/oxford_postdoc/romesh_nsys/sweeps',self.original_folder,'results.mat'));

			inputs.original_c = d.inputs.c;
			inputs.original_v = d.inputs.v;
			inputs.original_coupling = d.inputs.coupling;
			inputs.original_velocity = d.inputs.velocity;
			inputs.original_task_ids = d.task_ids;
		end
		...
		function out = do_work(self,task_id,inputs)
			...
			% First, get the input c and v
			this_coupling = inputs.c(task_id);
			this_velocity = inputs.v(task_id);

			% Was there an old run? If so, copy from the old one
			idx = find(inputs.original_c(:) == this_coupling & inputs.original_v(:) == this_velocity);
			if ~isempty(idx)
				oldfile = fullfile('/vols/Scratch/romesh/oxford_postdoc/romesh_nsys/sweeps',self.original_folder,sprintf('results_part_%d.mat',inputs.original_task_ids(idx)));
				copyfile(oldfile,this_run_fname)
				out = struct('completed',true); % Skip saving outputs
				return
			end
